...
class Spot:
    ...
    def show(self, color=BLACK):
        if self.walls[0]:
            pygame.draw.line(screen, color, [round(self.x * hr), round(self.y * wr)],
                             [round(self.x * hr + hr), round(self.y * wr)], 2)
        if self.walls[1]:
            pygame.draw.line(screen, color, [round(self.x * hr + hr), round(self.y * wr)],
                             [round(self.x * hr + hr), round(self.y * wr + wr)], 2)
        if self.walls[2]:
            pygame.draw.line(screen, color, [round(self.x * hr + hr), round(self.y * wr + wr)],
                             [round(self.x * hr), round(self.y * wr + wr)], 2)
        if self.walls[3]:
            pygame.draw.line(screen, color, [round(self.x * hr), round(self.y * wr + wr)],
                             [round(self.x * hr), round(self.y * wr)], 2)
...
while not done:
    ...
    if not completed:
        ...
        for i in range(rows):
            for j in range(cols):
                grid[i][j].show(BLACK)  # Цвет стен
                # grid[i][j].show_block(RED)

        current.visited = True
        current.show_block(BLUE)  # Цвет блока
        pygame.display.flip()
